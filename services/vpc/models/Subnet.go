...
type Subnet struct {
    ...
    /* 子网网段内可用内网IP数，包含按单IP和按网段分配的内网IP (Optional) */
    AvailableIpCount int `json:"availableIpCount"`

    /* 预分配网段内可用IP数，在子网开启预分配网段功能后生效 (Optional) */
    AvailableCidrIpCount int `json:"availableCidrIpCount"`
    ...
    /* 子网的ipv6起始地址 (Optional) */
    Ipv6StartIp string `json:"ipv6StartIp"`

    /* 子网的ipv6结束地址 (Optional) */
    Ipv6EndIp string `json:"ipv6EndIp"`
    ...
    /* 子网是否为外部子网（即子网路由表中存在下一跳为internet的路由或开启了eip优先访问）。true表示外部子网，false表示内部子网 (Optional) */
