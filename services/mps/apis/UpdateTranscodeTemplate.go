// Copyright 2018 JDCLOUD.COM
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
//
// NOTE: This class is auto generated by the jdcloud code generator program.

package apis

import (
    "github.com/jdcloud-api/jdcloud-sdk-go/core"
    mps "github.com/jdcloud-api/jdcloud-sdk-go/services/mps/models"
)

type UpdateTranscodeTemplateRequest struct {

    core.JDCloudRequest

    /* 模板ID  */
    TemplateId int `json:"templateId"`

    /* 模板标题。长度不超过 128 个字符，最少 2 个字符。UTF-8 编码。
 (Optional) */
    Title *string `json:"title"`

    /* 视频参数配置 (Optional) */
    Video *mps.VideoStreamSettings `json:"video"`

    /* 音频参数配置 (Optional) */
    Audio *mps.AudioStreamSettings `json:"audio"`

    /* 封装容器配置 (Optional) */
    Container *mps.ContainerSettings `json:"container"`

    /* 加密配置 (Optional) */
    Encryption *mps.EncryptionSettings `json:"encryption"`

    /* 清晰度规格标记。取值范围：
  SD - 标清
  HD - 高清
  FHD - 超清
  2K
  4K
 (Optional) */
    Definition *string `json:"definition"`

    /* 转码方式。取值范围：
  normal - 普通转码
  jdchd - 京享超清
  jdchs - 极速转码
 (Optional) */
    TranscodeType *string `json:"transcodeType"`
}

/*
 * param templateId: 模板ID (Required)
 *
 * @Deprecated, not compatible when mandatory parameters changed
 */
func NewUpdateTranscodeTemplateRequest(
    templateId int,
) *UpdateTranscodeTemplateRequest {

	return &UpdateTranscodeTemplateRequest{
        JDCloudRequest: core.JDCloudRequest{
			URL:     "/transcodeTemplates/{templateId}",
			Method:  "PUT",
			Header:  nil,
			Version: "v1",
		},
        TemplateId: templateId,
	}
}

/*
 * param templateId: 模板ID (Required)
 * param title: 模板标题。长度不超过 128 个字符，最少 2 个字符。UTF-8 编码。
 (Optional)
 * param video: 视频参数配置 (Optional)
 * param audio: 音频参数配置 (Optional)
 * param container: 封装容器配置 (Optional)
 * param encryption: 加密配置 (Optional)
 * param definition: 清晰度规格标记。取值范围：
  SD - 标清
  HD - 高清
  FHD - 超清
  2K
  4K
 (Optional)
 * param transcodeType: 转码方式。取值范围：
  normal - 普通转码
  jdchd - 京享超清
  jdchs - 极速转码
 (Optional)
 */
func NewUpdateTranscodeTemplateRequestWithAllParams(
    templateId int,
    title *string,
    video *mps.VideoStreamSettings,
    audio *mps.AudioStreamSettings,
    container *mps.ContainerSettings,
    encryption *mps.EncryptionSettings,
    definition *string,
    transcodeType *string,
) *UpdateTranscodeTemplateRequest {

    return &UpdateTranscodeTemplateRequest{
        JDCloudRequest: core.JDCloudRequest{
            URL:     "/transcodeTemplates/{templateId}",
            Method:  "PUT",
            Header:  nil,
            Version: "v1",
        },
        TemplateId: templateId,
        Title: title,
        Video: video,
        Audio: audio,
        Container: container,
        Encryption: encryption,
        Definition: definition,
        TranscodeType: transcodeType,
    }
}

/* This constructor has better compatible ability when API parameters changed */
func NewUpdateTranscodeTemplateRequestWithoutParam() *UpdateTranscodeTemplateRequest {

    return &UpdateTranscodeTemplateRequest{
            JDCloudRequest: core.JDCloudRequest{
            URL:     "/transcodeTemplates/{templateId}",
            Method:  "PUT",
            Header:  nil,
            Version: "v1",
        },
    }
}

/* param templateId: 模板ID(Required) */
func (r *UpdateTranscodeTemplateRequest) SetTemplateId(templateId int) {
    r.TemplateId = templateId
}
/* param title: 模板标题。长度不超过 128 个字符，最少 2 个字符。UTF-8 编码。
(Optional) */
func (r *UpdateTranscodeTemplateRequest) SetTitle(title string) {
    r.Title = &title
}
/* param video: 视频参数配置(Optional) */
func (r *UpdateTranscodeTemplateRequest) SetVideo(video *mps.VideoStreamSettings) {
    r.Video = video
}
/* param audio: 音频参数配置(Optional) */
func (r *UpdateTranscodeTemplateRequest) SetAudio(audio *mps.AudioStreamSettings) {
    r.Audio = audio
}
/* param container: 封装容器配置(Optional) */
func (r *UpdateTranscodeTemplateRequest) SetContainer(container *mps.ContainerSettings) {
    r.Container = container
}
/* param encryption: 加密配置(Optional) */
func (r *UpdateTranscodeTemplateRequest) SetEncryption(encryption *mps.EncryptionSettings) {
    r.Encryption = encryption
}
/* param definition: 清晰度规格标记。取值范围：
  SD - 标清
  HD - 高清
  FHD - 超清
  2K
  4K
(Optional) */
func (r *UpdateTranscodeTemplateRequest) SetDefinition(definition string) {
    r.Definition = &definition
}
/* param transcodeType: 转码方式。取值范围：
  normal - 普通转码
  jdchd - 京享超清
  jdchs - 极速转码
(Optional) */
func (r *UpdateTranscodeTemplateRequest) SetTranscodeType(transcodeType string) {
    r.TranscodeType = &transcodeType
}


// GetRegionId returns path parameter 'regionId' if exist,
// otherwise return empty string
func (r UpdateTranscodeTemplateRequest) GetRegionId() string {
    return ""
}

type UpdateTranscodeTemplateResponse struct {
    RequestID string `json:"requestId"`
    Error core.ErrorResponse `json:"error"`
    Result UpdateTranscodeTemplateResult `json:"result"`
}

type UpdateTranscodeTemplateResult struct {
    TemplateId string `json:"templateId"`
    Title string `json:"title"`
    Video mps.VideoStreamSettings `json:"video"`
    Audio mps.AudioStreamSettings `json:"audio"`
    Container mps.ContainerSettings `json:"container"`
    Encryption mps.EncryptionSettings `json:"encryption"`
    Definition string `json:"definition"`
    TranscodeType string `json:"transcodeType"`
    CreateTime string `json:"createTime"`
    UpdateTime string `json:"updateTime"`
}